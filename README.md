# ConciergeBot — ACP 2.0 Subscription Boilerplate
...
| Any subscription / "watch X" / scheduled push offering | ACP_ConciergeBot |
| Both shapes in one bot | ACP_ConciergeBot (handles both) |

## Architecture

```
acp-v2/   (Node 22 / TypeScript)            ConciergeBot.Api/   (.NET 10)
...
```powershell
$env:CONCIERGEBOT_ALLOW_UNAUTHENTICATED_DEV = "true"
# AND for the sidecar's streamPush server:
$env:CONCIERGEBOT_ALLOW_UNAUTHENTICATED_STREAM_PUSH = "true"
```
...
(Same 7 acceptance tests as in `docs/superpowers/specs/2026-05-03-acp-conciergebot-boilerplate-design.md`.)
...
1. Copy `ACP_ConciergeBot/` → `ACP_MyNewBot/`
2. Find/replace `ConciergeBot` → `MyNewBot` (case-sensitive) in: folder, .sln, .csproj, namespaces, package.json `name`, docker-compose service/container names, env var names (`CONCIERGEBOT_*` → `MYNEWBOT_*`).
...
- `streamPush` (sidecar internal HTTP server) binds `127.0.0.1` by default (audit F2, 2026-05-25). The Docker compose deploy sets `CONCIERGEBOT_STREAM_PUSH_BIND_HOST=0.0.0.0` so the API container can reach it across the docker bridge; the bridge is the trust boundary. Never publish the stream-push port to the host.
- `streamPush` REFUSES to start without `CONCIERGEBOT_API_KEY` (audit F6, 2026-05-25). The previous behaviour was "auth-required-when-set, silent-unauth-otherwise", which regressed to unauthenticated whenever the env var was forgotten. Now an operator who genuinely wants unauth (local dev where the C# tier is also unauthenticated) opts in by name via `CONCIERGEBOT_ALLOW_UNAUTHENTICATED_STREAM_PUSH=true`.
- `streamPush` also runs its own per-remote-address sliding-window rate limit (audit F8, 2026-05-25) — 60 req/min default; override via `CONCIERGEBOT_STREAM_PUSH_RATE_LIMIT`. Independent from the C# tier's `RateLimitMiddleware`; defends against a runaway loop in `InJobStreamDeliveryService` and against any caller that reaches the docker bridge IP.
- `WebhookSecretCipher` (AES-256-GCM at rest) wraps `webhook_secret` on Insert and unwraps on Read (audit F3, 2026-05-25). Required by default in non-Development — `Program.cs` fails fast at boot unless `WEBHOOK_SECRET_ENCRYPTION_KEY` is set (32 random bytes, base64), or the operator explicitly opts into plaintext via `CONCIERGEBOT_ALLOW_PLAINTEXT_WEBHOOK_SECRETS=true` (transitional only — emits a loud boot warning). Generate the key with `openssl rand -base64 32`. Migration is lazy: rows that pre-date the cipher decode as-is until their next write.
...
See `docs/superpowers/specs/2026-05-03-acp-conciergebot-boilerplate-design.md` for full design.
