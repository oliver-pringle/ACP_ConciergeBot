...
// unset CONCIERGEBOT_API_KEY (the C# tier already fails fast in non-
...
// CONCIERGEBOT_ALLOW_UNAUTHENTICATED_STREAM_PUSH=true is set (for
...
// CONCIERGEBOT_STREAM_PUSH_RATE_LIMIT). The C# tier already has its
...
// multi-container deploy MUST set CONCIERGEBOT_STREAM_PUSH_BIND_HOST=0.0.0.0
...
// CONCIERGEBOT_STREAM_PUSH_RATE_LIMIT (integer, requests per minute).
...
export function startStreamPushServer(opts: StreamPushServerOptions): Server {
  ...
  // escape hatch CONCIERGEBOT_ALLOW_UNAUTHENTICATED_STREAM_PUSH=true is
  ...
  const allowUnauthenticated =
    (process.env.CONCIERGEBOT_ALLOW_UNAUTHENTICATED_STREAM_PUSH ?? "").toLowerCase() === "true";
  if (!apiKey && !allowUnauthenticated) {
    throw new Error(
      `[streamPush] REFUSING to start without CONCIERGEBOT_API_KEY. ` +
      `/v1/internal/push-tick + /v1/internal/submit-final would be unauthenticated. ` +
      `Set CONCIERGEBOT_API_KEY, or set ` +
      `CONCIERGEBOT_ALLOW_UNAUTHENTICATED_STREAM_PUSH=true to explicitly opt into ` +
      `unauth mode for local-only dev (do NOT set this in any deployed environment).`);
  }
  if (!apiKey && allowUnauthenticated) {
    console.warn(
      `[streamPush] CONCIERGEBOT_ALLOW_UNAUTHENTICATED_STREAM_PUSH=true — ` +
  ...
  const rateLimitPerMin = (() => {
    const raw = process.env.CONCIERGEBOT_STREAM_PUSH_RATE_LIMIT;
