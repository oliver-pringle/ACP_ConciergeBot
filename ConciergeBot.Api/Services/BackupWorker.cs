...
// P6: daily online SQLite backup using Microsoft.Data.Sqlite's BackupDatabase API
// (SQLite Online Backup, page-level locking, WAL-aware). Writes to a temp file then
// renames atomically so a mid-backup crash never produces a torn file. Rotation is
// filename-based: KeepDays counts unique YYYY-MM-DD stamps. The bot DB runs in WAL
// mode, so a naive `cp` would miss the -wal file. Portfolio convention (lifted from
// ACP_LiquidGuard); needs /data/backups on the same bind-mount as the live .db.
public class BackupWorker : BackgroundService
{
    ...
    public BackupWorker(IConfiguration config, Db db, ILogger<BackupWorker> logger)
    {
        ...
        _baseName = config["Backup:BaseName"] ?? "conciergebot";
    }
