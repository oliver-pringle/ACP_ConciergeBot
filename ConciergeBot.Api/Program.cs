...
// an operator-controlled URL (CONCIERGEBOT_STREAM_PUSH_URL), NOT a
...
// no network call, no secret required. The LLM is used ONLY to narrate an
// already-determined deterministic route (RouteNarrator) — never to choose
// offerings, prices, or steps. Virtuals Compute is reached through the same
// OpenAI-compatible client. See docs/conciergebot/compute-smoke.md.
var llmOptions = LlmOptions.FromConfiguration(builder.Configuration);
builder.Services.AddSingleton(llmOptions);
if (llmOptions.Provider == LlmProvider.Disabled)
{
    builder.Services.AddSingleton<ILlmClient>(new DisabledLlmClient(llmOptions));
}
else
{
    // ResponseHeadersRead is applied per-call inside the client; the typed
    // HttpClient just carries the timeout so a hung compute endpoint can't
    // stall a route_stack response indefinitely.
    builder.Services.AddHttpClient<ILlmClient, OpenAiCompatibleLlmClient>(c =>
    {
        c.Timeout = TimeSpan.FromSeconds(llmOptions.TimeoutSeconds);
    });
}
...
// LLM posture banner — conscious on every restart. The key value is NEVER
// logged, only present/absent. When a network provider is configured but no
// key is present, warn (narration will fail closed to deterministic summaries).
var bootLlm = app.Services.GetRequiredService<LlmOptions>();
app.Logger.LogInformation(
    "[llm] provider={Provider} endpoint={Endpoint} model={Model} key={KeyState}",
    bootLlm.ProviderLabel,
    bootLlm.Provider == LlmProvider.Disabled ? "(n/a)" : bootLlm.Endpoint,
    bootLlm.Model,
    string.IsNullOrEmpty(bootLlm.ApiKey) ? "absent" : "present");
if (bootLlm.Provider != LlmProvider.Disabled && string.IsNullOrEmpty(bootLlm.ApiKey))
...
if (!app.Environment.IsDevelopment())
{
    ...
    // deploys only) via CONCIERGEBOT_ALLOW_PLAINTEXT_WEBHOOK_SECRETS=true.
    var webhookCipher = app.Services.GetRequiredService<WebhookSecretCipher>();
    var allowPlaintextSecrets = string.Equals(
        builder.Configuration["CONCIERGEBOT_ALLOW_PLAINTEXT_WEBHOOK_SECRETS"]
            ?? Environment.GetEnvironmentVariable("CONCIERGEBOT_ALLOW_PLAINTEXT_WEBHOOK_SECRETS"),
        "true", StringComparison.OrdinalIgnoreCase);
    if (!webhookCipher.IsEncryptionEnabled && !allowPlaintextSecrets)
        throw new InvalidOperationException(
            ...
            "CONCIERGEBOT_ALLOW_PLAINTEXT_WEBHOOK_SECRETS=true for a transitional deploy.");
    if (!webhookCipher.IsEncryptionEnabled && allowPlaintextSecrets)
        app.Logger.LogWarning(
            "CONCIERGEBOT_ALLOW_PLAINTEXT_WEBHOOK_SECRETS=true — webhook_secret column persists plaintext. " +
...
// the explicit escape hatch CONCIERGEBOT_ALLOW_UNAUTHENTICATED_DEV=true
...
var apiKey = builder.Configuration["ApiKey"]
    ?? Environment.GetEnvironmentVariable("CONCIERGEBOT_API_KEY");
var allowUnauthenticatedDev = string.Equals(
    builder.Configuration["CONCIERGEBOT_ALLOW_UNAUTHENTICATED_DEV"]
        ?? Environment.GetEnvironmentVariable("CONCIERGEBOT_ALLOW_UNAUTHENTICATED_DEV"),
    "true", StringComparison.OrdinalIgnoreCase);
if (string.IsNullOrEmpty(apiKey))
{
    ...
    {
        throw new InvalidOperationException(
            "CONCIERGEBOT_API_KEY is required in non-Development environments. " +
    ...
    {
        throw new InvalidOperationException(
            "CONCIERGEBOT_API_KEY is required even in Development. " +
            "Set the env var (or `ApiKey`) to any string for local dev, or set " +
            "CONCIERGEBOT_ALLOW_UNAUTHENTICATED_DEV=true to explicitly opt into unauth mode.");
    }
    app.Logger.LogWarning(
        "CONCIERGEBOT_API_KEY not set + CONCIERGEBOT_ALLOW_UNAUTHENTICATED_DEV=true — Development unauth mode. " +
